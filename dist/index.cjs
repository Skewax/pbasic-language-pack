...
function getStyleTags() {
    let out = {
        ...
        Char: highlight.tags.string,
        Comment: highlight.tags.lineComment,
        Number: highlight.tags.number
    };
    kwTypes.forEach(x => out[x] = highlight.tags.keyword);
    formatKWTypes.forEach(x => out[x] = highlight.tags.keyword);
    return out;
}
/**
 * The PBasic language instance
 */
const PBasic = language.LRLanguage.define({
    name: 'pbasic',
    parser: parser.configure({
        props: [
            highlight.styleTags(getStyleTags()),
            language.foldNodeProp.add({
                Loop: language.foldInside,
                UntilLoop: language.foldInside,
                WhileLoop: language.foldInside,
                For: language.foldInside,
                If: language.foldInside,
                Elseif: language.foldInside,
                Else: language.foldInside,
                Select: language.foldInside,
                Case: language.foldInside,
                CaseElse: language.foldInside,
...
function completions(context) {
    ...
    return autocomplete.completeFromList(pbasicCompletionMap.concat(defCompletes))(context);
}
/**
...
const pbasicHover = view.hoverTooltip((view, pos, side) => {
    // TODO: this better
 ...
 */
const pbasicLinter = lint.linter(view => {
    let diagnostics = [];
    // Get the definitions.
    // TODO: This should be memoized, if possible.
    const tree = language.syntaxTree(view.state);
 ...
 */
function pbasic() {
    return new language.LanguageSupport(PBasic, [pbasicCompletion, pbasicLinter, pbasicHover]);
}

exports.PBasic = PBasic;
exports.pbasic = pbasic;
exports.pbasic_parser = pbasic_parser;
exports.typeHoverDOMProvider = typeHoverDOMProvider;
